...
void upsampling (int bpskMod){

    for (int i=0; i<*bpskARR; i++){
        for(int j=0; j< upsampleARR[i];j+10){
            if (j%10 ==0){
...
int main(){
    //int formfilter(*upsampleARR)
    // for(size_t i = 0; i < 2; i++)
    //     {
    //         upsampleARR[0 + i] = 0xffff;
    //         // 8 x timestamp words
    //         upsampleARR[10 + i] = 0xffff;
    ...
    //         upsampleARR[2 + i] = tx_time_byte << 4;
    //     }
    ...
    int16_t *my_file = read_pcm("/home/plutoSDR/Рабочий стол/SDRlabs/dev/tx_audio.pcm", &sample_count); //lhgjk]=ed0thnj
    ...
    FILE *file = fopen("txdata.pcm", "w");//ihgoyoyovy
